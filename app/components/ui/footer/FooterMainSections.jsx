    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 mb-10 text-center md:text-left">
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
        ...
        <p className="leading-relaxed text-gray-600 dark:text-gray-400">
          We are a modern electronics store offering high-quality laptops, smartphones, and accessories with fast delivery and trusted service.
        </p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
        ...
        <ul className="space-y-3">
          {categories.map((cat) => (
            <li key={cat.name}>
              <Link href={cat.to} className="transition hover:text-red-500">{cat.name}</Link>
            </li>
          ))}
        </ul>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
        ...
        <ul className="space-y-3">
          {informationLinks.map((item) => (
            <li key={item.info}>
              <Link href={item.to} className="transition hover:text-red-500">{item.info}</Link>
            </li>
          ))}
        </ul>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}>
        ...
        <ul className="space-y-3">
          {customerServices.map((service) => (
            <li key={service.info}>
              <Link href={service.to} className="transition hover:text-red-500">{service.info}</Link>
            </li>
          ))}
        </ul>
      </motion.div>
